...
export class Logger implements ILogger {
  ...
  private defaultSettings(): ILoggerOptions {
    return {
      global: {
        ...
        template: () => DefaultOptionConstants.BASE_TEMPLATE,
      },
      theme: {
        info: {
          level: {
  ...
  private universalLog(level: LogLevelType, ...content: unknown[]): void {
    ...
    const templateFunc =
      ...
      ] as unknown as TLoggerTemplate) || global?.template;
  ...
  private getDate(): string {
    const format = this.options.formatting?.dateFormat || "D MMM YYYY HH:mm:ss";
